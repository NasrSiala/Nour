...
import {
  ...
  Menu, X, AlertTriangle, ChevronRight, ClipboardCheck,
} from "lucide-react";
...
export function TeacherLayout({ children }: { children: React.ReactNode }) {
  ...
  const isActive = (href: string) =>
    href === "/teacher" ? location === href : location.startsWith(href);
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row" style={{ backgroundColor: "#f4f4f0" }}>
      ...
      {/* Sidebar */}
      <div
        className={`${isMobileMenuOpen ? "flex" : "hidden"} md:flex flex-col w-full md:w-56 flex-shrink-0`}
      ...
      >
        <div className="hidden md:flex items-center gap-3 px-5 py-[22px]" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div
            ...
            style={{ backgroundColor: "rgba(255,255,255,0.1)", color: "#a7f3d0", fontFamily: "'Sora', sans-serif" }}
          >
            SB
          </div>
          <div>
            <p className="text-white font-semibold text-sm" style={{ fontFamily: "'Sora', sans-serif" }}>SchoolBox</p>
            <p style={{ color: "#4d7a62", fontSize: "10px", fontWeight: 600, letterSpacing: "0.06em" }}>TEACHER PORTAL</p>
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-2.5" style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          {navGroups.map((group) => (
            <div key={group.label}>
              <p style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.1em", color: "#3d6650", padding: "0 10px", marginBottom: "4px", textTransform: "uppercase" }}>
                {group.label}
              </p>
                  ...
                  <Link key={item.name} href={item.href}>
                    <div
                      ...
                      style={{
                        ...
                        backgroundColor: active ? SIDEBAR_ACTIVE_BG : "transparent", marginBottom: "1px", position: "relative",
                      }}
                    ...
                    >
                      {active && (
                        <span style={{ position: "absolute", left: 0, top: "50%", transform: "translateY(-50%)", width: "3px", height: "18px", backgroundColor: "#4ade80", borderRadius: "0 2px 2px 0" }} />
                      )}
                      <item.icon style={{ width: "15px", height: "15px", flexShrink: 0, color: active ? "#a7f3d0" : "#4d7a62" }} />
                      <span style={{ fontSize: "13.5px", fontWeight: active ? 600 : 500, color: active ? "#ffffff" : "#7da890", flex: 1 }}>
                        {item.name}
                      </span>
        ...
        <div style={{ padding: "12px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px 10px", borderRadius: "8px", marginBottom: "2px" }}>
            <div style={{ width: "30px", height: "30px", borderRadius: "50%", backgroundColor: "#1a4d30", border: "1.5px solid rgba(74,222,128,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "11px", fontWeight: 700, color: "#a7f3d0", flexShrink: 0, fontFamily: "'Sora', sans-serif" }}>
              {initials}
            </div>
